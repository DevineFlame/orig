...
////////////////////////////////////calls to server/////////////////////////////////
    ...
    socket.on('candidate',handle_candidate);
    
/////////////////////////////////functions for webrtc/////////////////////////////////
 
 function handle_offer(data){
    var offer=data.offer;
    var user=data.user;
    var room=data.room;
    log(user+" send offer to "+room);
    socket.broadcast.to(room).emit('offer',{'offer':offer,'user':user,'room':room});
 ...
 function handle_answer(data){
    ...
    var creator=data.creator;
     log(user+" giving answer  to "+creator);
    socket.broadcast.to(creator).emit('answer',{'answer':answer,'creator':creator,'user':user,'room':room});
 ...
 function handle_candidate(data){
    ...
    var user=data.user;
    log("sender of candidate is  --->"+user);
    socket.broadcast.to('root').emit('candidate',{'candidate':candidate,'user':user});
 }
